...
(function($) {
	$.fn.stellarNav = function(options, width, breakpoint) {
		...
		var settings = $.extend( {
			...
			childClosingDelay: 250, // controls how long the child dropdowns stay open for in milliseconds
				...
				$(parentItems).each(function() {
					if( $(this).hasClass('mega') ) {
						...
						$(this).find('li.has-sub').on('mouseleave', function(){
							$(this).children('ul').stop(true, true).delay(settings.childClosingDelay).slideUp(settings.openingSpeed);
						});
					}
				});
			}
